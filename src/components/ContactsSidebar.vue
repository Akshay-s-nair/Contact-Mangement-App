<template>
  <div :class="['sidebar', { expanded: isExpanded }]">
    ...
    <div class="profileImg">
      <img v-if="isMobileView && isExpanded || !isMobileView" class="profile-picture" :src="`/profile_pictures/${img}`" alt="Profile Picture" />
    </div>
    ...
    <button class="toggle-bt" @click="toggleSidebar" v-if="isExpanded && !isMobileView">
      ...
      <div v-if="isMobileView && isExpanded" class="ContactNavsPar">
        ...
        <div v-if="success" class="alert alert-success">{{ success }}</div>
          ...
          <button class="ContactNavs" @click="showUserDetails">User</button>
...
export default {
  ...
  data() {
    return {
      ...
      isMobileView: window.innerWidth <= 768 // Check if it's mobile view
    };
  },
  computed: {
